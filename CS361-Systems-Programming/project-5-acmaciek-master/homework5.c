...
char * request_html = "HTTP/1.0 200 OK\r\nContent-type: text/html; charset=UTF-8\r\n\r\n";
...
char * request_plain = "HTTP/1.0 200 OK\r\nContent-type: text/plain; charset=UTF-8\r\n\r\n";

char * error_404 = "HTTP/1.0 404 Not Found\r\n"
"Content-Type: text/html; charset=UTF-8\r\n\r\n %s";
char * drake = "<!DOCTYPE html>\n"
"<html>\n"
"<body>\n"
"\n"
"<h1 style=\"color:orange\">ERROR 404 FILE NOT FOUND</h1>\n"
"<img src=\"https://i.imgur.com/GpY6bTJ.png\" style=\"width:640px;height:428px;\">\n"
"\n"
"</body>\n"
"</html>";
...
//copied from original serve_request
//send data from the filename
void send_client(int client_fd, char* filename)
{
...
char * get_404 (char * file)
{
    char * save = malloc(512);
    sprintf(save, error_404, drake);
    return save;
}
...
void serve_request(int client_fd){
    ...
    {
        send(client_fd, get_404(filename), strlen(get_404(filename)), 0);
    }

    else if(S_ISDIR(file_stat.st_mode)) // Is a directory
    {
        char index_file[4096];
        sprintf(index_file, "%s%s", filename, "/index.html");
        
        if(stat(index_file, &file_stat) == 0) //if file at given index exists
        {
            char* type = request_html;
            send(client_fd, type, strlen(type), 0);
            send_client(client_fd, index_file);
        }
    }
    else // show it
    {
        char *extension = strrchr(filename, '.') + 1; //file extension
        char *type = request_plain;
        if(strcmp("404", extension) == 0) type = request_404;
        else if(strcmp("html", extension) == 0) type = request_html;
        else if(strcmp("jpg", extension) == 0) type = request_jpeg;
        else if(strcmp("gif", extension) == 0) type = request_gif;
        else if(strcmp("png", extension) == 0) type = request_png;
        else if(strcmp("pdf", extension) == 0) type = request_pdf;
        send(client_fd, type, strlen(type), 0);
        send_client(client_fd, filename);
    }
    close(client_fd);
    return;
}
